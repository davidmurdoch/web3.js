...
/**
 * In Node.js, calling `unref(timer)` on a running timer ensures that the timer
 ...
 * @param { NodeJS.Timeout | number } timer
 * @returns { boolean } `true` if the timer could be `unref`ed, otherwise returns `false`
 */
...
var Method = function Method(options) {
    ...
    this.transactionPollingInterval =
        options.transactionPollingInterval || 1000;
    ...
    var startWatching = function (existingReceipt) {
        ...
        const startInterval = () => {
            intervalId = setInterval(
                ...
                method.transactionPollingInterval
            );
            unref(intervalId); // prevent this interval from keeping Node.js processes running
        };
        ...
        unref(blockHeaderTimeoutId); // prevent this timeout from keeping Node.js processes running
    }.bind(this);
...
Method.prototype.buildCall = function () {
    ...
    var send = function () {
        ...
        if (method.call === "eth_getTransactionReceipt") {
            method.hexFormat =
                payload.params.length < args.length &&
